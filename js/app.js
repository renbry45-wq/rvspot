...
document.addEventListener('DOMContentLoaded',()=>{const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';s.onload=()=>{const sb=window.supabase.createClient('https://uydiifdgjzylfxxaoznv.supabase.co','eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV5ZGlpZmRnanp5bGZ4eGFvem52Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwOTMxNDMsImV4cCI6MjA5MDY2OTE0M30.McVsij-Z3m-rh9wjedwl5hM5fAaB5YDGYfHjsgxDfdE');window._sb=sb;document.querySelectorAll('button').forEach(b=>{if(b.textContent.trim()==='Continue with Google')b.onclick=()=>sb.auth.signInWithOAuth({provider:'google',options:{redirectTo:'https://rvspot.net'}});});sb.auth.getUser().then(({data:{user}})=>{if(user){const a=document.querySelector('.nav-actions');if(a)a.innerHTML=`<span style="font-size:13px;color:var(--slate-600)">Hi, ${user.user_metadata?.first_name||user.email.split('@')[0]}</span><a href="/pages/user-dashboard" class="btn btn-secondary btn-sm">Dashboard</a><button class="btn btn-ghost btn-sm" onclick="window._sb.auth.signOut().then(()=>location.reload())">Sign out</button>`;}});};document.head.appendChild(s);});
